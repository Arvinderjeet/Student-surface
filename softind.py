...
def exit_button():
    ...
    time.sleep(1)
    exit()
copyright = Label(tk,text="Made with \u2764 by Arvinderjeet singh ", font=("lucida calligraphy", 16, ""), bg=None, fg="#455443", bd="0")
...
def ncert():
      ...
      print('│OPENING FILE |'+str(os.system(userInput)))
def calc_win():
    calc_new = threading.Thread(target=calc, daemon=True)
    calc_new.start()
def board_win():
    calc_new = threading.Thread(target=board, daemon=True)
    calc_new.start()
def ncert_win():
    calc_new = threading.Thread(target=ncert, daemon=True)
    calc_new.start()
def side_left():
    # ================CALCULATOR==========
...
def pdf_viewer_win():
    pdf_viewer_new = threading.Thread(target=pdf_viewer, daemon=True)
    pdf_viewer_new.start()
...
def right_side():
    ...
    def pw():
        calc_new = threading.Thread(target=pw1, daemon=True)
        calc_new.start()
    def vedantu1():
        webbrowser.open_new(r"https://www.vedantu.com/")
    def vedantu():
        calc_new = threading.Thread(target=vedantu1, daemon=True)
        calc_new.start()
    def unacademy1():
        webbrowser.open_new(r"https://unacademy.com/")
    def unacademy():
        calc_new = threading.Thread(target=unacademy1, daemon=True)
        calc_new.start()
